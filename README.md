...
В профиле основного приложения появилась кнопка «Перейти к созданию». При первом открытии клиент создаёт уникальный идентификатор пользователя и сохраняет его в браузере. При нажатии кнопки фронтенд отправляет запрос `POST /auth/token` с этим идентификатором и получает токен, после чего формирует ссылку вида `https://example.com/admin?token=...`. После открытия ссылки:
...
- `POST /auth/token` — выдача токена администратора. Ожидает JSON `{ "userId": "...", "profile": { "displayName": "..." } }`.
