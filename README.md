...
Во время сборки автоматически применяются миграции Prisma и выполняется сидер с тестовыми данными.
При запуске контейнера сервер повторно применяет миграции (на случай примонтированного внешнего диска) и запускает сидер только
если файл `dev.db` отсутствует — так можно смело подключать пустую директорию и получать готовую тестовую базу прямо в volume.
...
Приложение будет доступно по адресу `http://localhost:4000/`, база данных `dev.db` появится в папке `api/prisma` вашего проекта
(если вы примонтировали volume).
Если хотите сохранить данные между перезапусками контейнера, оставьте volume подключённым — при старте сервер просто применит
миграции к существующему файлу и не будет пересоздавать его.

Фронтенд по умолчанию обращается к API по пути `/api`, поэтому значение `VITE_API_BASE_URL=/api` из `.env` подходит как для режима разработки, так и для Docker-сборки.

> **Примечание.** Сервер автоматически считывает переменные окружения из `api/.env` (а при наличии и из корневого `.env`) и не переопределяет значения, явно заданные при запуске контейнера через `-e` или системные переменные. Поэтому достаточно добавить `api/.env` перед сборкой образа, чтобы Telegram-бот и остальные настройки оказались доступны внутри Docker без дополнительных флагов.
