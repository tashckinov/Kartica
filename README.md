...
5. Добавьте домен приложения в список разрешённых доменов Telegram Login Widget через `@BotFather` (`/setdomain`).

API автоматически запрашивает имя бота через метод `getMe` и отдаёт его клиенту по маршруту `GET /auth/telegram-config`, поэтому на фронтенде не требуется дополнительных переменных окружения для отображения кнопки авторизации.
